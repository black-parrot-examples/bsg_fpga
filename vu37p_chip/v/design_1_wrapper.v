...
*/
 ...
 #(.width_p(1)
  ...
  assign GW_IC0_TAG_EN = tag_trace_en_r_lo[0];
  ...
  bsg_chip_swizzle_adapter
    swizzle
      (.port_ci_clk_i   (IC0_GW_CL_CLK)
      ,.port_ci_v_i     (IC0_GW_CL_V)
      ,.port_ci_data_i  ({IC0_GW_CL_D8, IC0_GW_CL_D7, IC0_GW_CL_D6, IC0_GW_CL_D5, IC0_GW_CL_D4, IC0_GW_CL_D3, IC0_GW_CL_D2, IC0_GW_CL_D1, IC0_GW_CL_D0})
      ,.port_ci_tkn_o   (IC0_GW_CL_TKN)

      ,.port_ci2_clk_o  (GW_IC0_CL_CLK)
      ,.port_ci2_v_o    (GW_IC0_CL_D4)
      ,.port_ci2_data_o ({GW_IC0_CL_D0, GW_IC0_CL_D1, GW_IC0_CL_D2, GW_IC0_CL_V, GW_IC0_CL_D3, GW_IC0_CL_D8, GW_IC0_CL_D7, GW_IC0_CL_D5, GW_IC0_CL_D6})
      ,.port_ci2_tkn_i  (GW_IC0_CL_TKN)
  ...
  bp_mem_noc_ral_link_s [ct_num_in_gp-1:0] prev_router_links_li, prev_router_links_lo;
  bp_mem_noc_ral_link_s [ct_num_in_gp-1:0] next_router_links_li, next_router_links_lo;
  ...
  for (genvar i = 0; i < ct_num_in_gp; i++)
  begin
    assign prev_ct_links_li[i].data          = prev_router_links_li[i].data;
    assign prev_ct_links_li[i].v             = prev_router_links_li[i].v;
    assign prev_ct_links_li[i].ready_and_rev = prev_router_links_li[i].ready_and_rev;
    assign prev_router_links_lo[i].data          = prev_ct_links_lo[i].data;
    assign prev_router_links_lo[i].v             = prev_ct_links_lo[i].v;
    assign prev_router_links_lo[i].ready_and_rev = prev_ct_links_lo[i].ready_and_rev;
  end
  ...
  for (genvar i = 0; i < ct_num_in_gp; i++)
  begin
    assign next_ct_links_li[i].data          = next_router_links_li[i].data;
    assign next_ct_links_li[i].v             = next_router_links_li[i].v;
    assign next_ct_links_li[i].ready_and_rev = next_router_links_li[i].ready_and_rev;
    assign next_router_links_lo[i].data          = next_ct_links_lo[i].data;
    assign next_router_links_lo[i].v             = next_ct_links_lo[i].v;
    assign next_router_links_lo[i].ready_and_rev = next_ct_links_lo[i].ready_and_rev;
  end
  ...
  assign next_router_links_li[0] = proc_cmd_link_li;
  assign next_router_links_li[1] = proc_resp_link_li;
  assign next_router_links_li[2] = dram_resp_link_li;
  
  assign proc_cmd_link_lo  = next_router_links_lo[0];
  assign proc_resp_link_lo = next_router_links_lo[1];
  assign dram_cmd_link_lo  = next_router_links_lo[2];
...
bp_me_cce_to_mem_link_bidir
 ...
 host_link
  (.clk_i(mig_clk)
   ,.reset_i((~tag_trace_done_lo | core_reset_lo))
...
logic nbf_done_lo, cfg_done_lo, dram_sel_lo;
  ...
  begin
    ...
    always_ff @(posedge mig_clk)
      begin
        if ((~tag_trace_done_lo | core_reset_lo))
          begin
            counter_r <= 1;
            nbf_done_r <= 0;
          end
...
bp_cce_mmio_cfg_loader
  ...
  cfg_loader
  (.clk_i(mig_clk)
   ,.reset_i((~tag_trace_done_lo | core_reset_lo) | ~dram_sel_lo)
...
always_comb
 ...
 #(.bp_params_p(bp_params_p)
  ...
  (.clk_i          (mig_clk)
  ,.reset_i        ((~tag_trace_done_lo | core_reset_lo) | (~cfg_done_lo & (load_nbf_p != 0)))
